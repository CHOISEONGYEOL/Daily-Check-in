...
const BOMB_RADIUS    = 120;  // 스플래시 범위 1.5x
...
export const WrBattle = {
    ...
    _battleRenderProjectiles(ctx, camX, camY) {
        ...
        for (const p of this._battleProjectiles) {
            ...
            // Screen-space culling using actual camera
            const screenX = p.x - cx, screenY = p.y - cy;
            if (screenX < -60 || screenX > this.VW + 60 || screenY < -60 || screenY > this.VH + 60) continue;
            // Draw at world coords (canvas transform handles camera)
    ...
    _battleRenderEffects(ctx, camX, camY) {
        ...
        if (this._battleExplosionRings) {
            ...
            for (let i = 0; i < this._battleExplosionRings.length; i++) {
                ...
                if (r.life > 0) {
                    ...
                    const screenX = r.x - cx, screenY = r.y - cy;
                    if (screenX < -r.maxRadius || screenX > this.VW + r.maxRadius ||
                        screenY < -r.maxRadius || screenY > this.VH + r.maxRadius) {
        ...
        if (this._battleMuzzleFlashes) {
            ...
            for (let i = 0; i < this._battleMuzzleFlashes.length; i++) {
                ...
                f.life--;
                if (f.life > 0) {
                    const screenX = f.x - cx, screenY = f.y - cy;
                    if (screenX < -80 || screenX > this.VW + 80 || screenY < -80 || screenY > this.VH + 80) {
    ...
    _battleRenderParticles(ctx, camX, camY) {
        ...
        for (const p of this._battleParticles) {
            ...
            // Screen-space culling
            const screenX = p.x - cx, screenY = p.y - cy;
            if (screenX < -40 || screenX > this.VW + 40 || screenY < -40 || screenY > this.VH + 40) continue;
    ...
    _battleRenderPickups(ctx, camX, camY) {
        ...
        for (const pk of this._battlePickups) {
            if (!pk.active) continue;
            // Screen-space culling
            const screenX = pk.x - cx;
            if (screenX < -30 || screenX > this.VW + 30) continue;
