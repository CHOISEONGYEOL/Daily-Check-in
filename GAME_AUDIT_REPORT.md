...
**작성일**: 2026-03-05 (초판) / 2026-03-06 (2차 감사, 3~5차 수정)
...
### [SC-H2] 축구: 호스트 전환 시 공 순간이동 -- ✅ 수정됨
- **파일**: `wr-realtime.js:857-865`
- **문제**: 새 호스트 선출 시 이전 호스트의 `ball._serverX/Y/Vx/Vy` 보정 잔량이 남아있어 새 호스트에서 공이 비정상 위치로 순간이동
- **수정 내용**: 호스트 전환 시 `ball._serverX/Y/Vx/Vy`를 `undefined`로 초기화하여 stale 보정 데이터 제거
- **커밋**: 5차 수정
...
### [BT-H3] 대전: `_rtOnRemoteGimmick`에서 배틀모드 처리 후 early return으로 다른 기믹 데이터 유실 -- ✅ 수정됨
- **파일**: `wr-realtime.js:1132-1165`
- **문제**: `data.battleMode`, `data.pickupTaken`, `data.pickupRespawn`, `data.battlePickupSync` 처리 시 각각 `return`으로 함수를 종료하여, 같은 페이로드에 포함된 다른 기믹 데이터가 무시됨
- **수정 내용**: 모든 early `return` 제거 → 조건부 처리 후 다음 블록으로 계속 진행하도록 변경
- **커밋**: 5차 수정

### [BT-H4] 대전: 스페이스바 점프만 차단, W/ArrowUp 점프는 허용 -- ✅ 수정됨
- **파일**: `waiting-room.js:274-276`
- **문제**: 배틀 모드에서 스페이스바는 사격으로 전환되지만, `ArrowUp`/`w`/`W`로는 여전히 점프 가능한 꼼수 존재
- **수정 내용**: 배틀 모드에서 모든 점프 키를 차단하고 스페이스바만 사격으로 동작하도록 조건문 통합
- **커밋**: 5차 수정

### [BT-H5] 대전: 팀 미배정 시 리스폰 위치 항상 오른쪽 편향 -- ✅ 수정됨
- **파일**: `wr-battle.js:519, 223`
- **문제**: `player.team === 'left' ? 200 : this.W - 200`으로 처리하여, `team`이 `null/undefined`이면 항상 오른쪽(`this.W - 200`)에서 리스폰
- **수정 내용**: 3항 조건으로 변경 — `left`→왼쪽, `right`→오른쪽, 그 외→맵 중앙(`this.W / 2`). 로컬 + 원격 리스폰 모두 수정
- **커밋**: 5차 수정

### [CM-H1] 공통: 스테이지 전환 시 원격 플레이어 `enteredDoor` 레이스 컨디션 -- ✅ 수정됨
- **파일**: `game.js:804`
- **문제**: 스테이지 전환 시 로컬에서 `enteredDoor = false`로 리셋하지만, 원격 클라이언트의 stale 위치 데이터(`enteredDoor: true`)가 도착하면 즉시 덮어써져 새 스테이지가 시작하자마자 클리어 판정 발생
- **수정 내용**: `_onGameRemotePos()`에서 원격 `enteredDoor` 데이터를 무시하도록 변경. 문 통과 판정은 로컬 충돌 검사(`updateDoor`)로만 관리
- **커밋**: 5차 수정
...
### 2026-03-06 (5차 수정)
나머지 High 5개 이슈 수정 완료 — **High 등급 전수 수정 달성**:
- **[SC-H2]** 호스트 전환 시 `ball._serverX/Y/Vx/Vy` 초기화로 공 순간이동 방지 (`wr-realtime.js`)
- **[CM-H1]** 원격 `enteredDoor` 데이터 무시 → 로컬 충돌 판정으로만 관리하여 스테이지 전환 레이스 컨디션 해결 (`game.js`)
- **[BT-H3]** `_rtOnRemoteGimmick`의 배틀 데이터 처리 후 early return 제거 → 기믹 데이터 유실 방지 (`wr-realtime.js`)
- **[BT-H4]** 배틀 모드에서 모든 점프 키(W/ArrowUp 포함) 차단 (`waiting-room.js`)
- **[BT-H5]** 팀 미배정 플레이어 리스폰 위치를 맵 중앙으로 변경 — 로컬+원격 모두 (`wr-battle.js`)
...
| **Critical** | 0 | 전부 수정 완료 |
| **High** | 0 | 전부 수정 완료 |
| **Medium** | 12 | SC-M1, SC-M5, SC-M6, BT-M1~M6, CM-M1~M3 |
| **Low** | 13 | SC-L1~L4, BT-L1~L5, CM-L1~L4 |
